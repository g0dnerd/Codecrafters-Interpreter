...
impl AstGenerator {
    ...
    pub fn define_ast(&self, base_name: &str) {
        ...
        stream.write(b"use crate::expression::Expression;\n\n").unwrap();
        stream.write(b"use crate::token::{ Token, LiteralValue };\n\n").unwrap();
        stream.flush().unwrap();

        for t in &self.types {
            let typedef = t.split_once(':')
                .expect("Invalid type definition: {t}");
    ...
    fn define_type(
        ...
        for field in field_vec {
            let field_def = field.split_once(' ')
